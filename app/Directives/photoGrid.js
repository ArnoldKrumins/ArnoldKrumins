...
ak.directive('photoGrid',function($timeout){
    ...
    return{
         ...
         template:'<div id="photo-container">' +
                    '<div class="card-wrapper" ng-repeat="n in range() track by $index">' +
                        '<div class="card" ng-click="flip($index)"  >' +
                            '<div class="box front" value="{{$index}}" stagger ng-style="imageSections[$index]">{{$index}}</div>' +
                            '<div class="box back" value="" ng-style="shuffledSections[$index]"></div>' +
                        '</div>' +
                     '</div>' +
                   '</div>',
